...
#define IMM_J int32_t imm = (int32_t)sign_extend(sub_bits(this->inst, 31, 31) << 20 | sub_bits(this->inst, 30, 21) << 1 | sub_bits(this->inst, 20, 20) << 11 | sub_bits(this->inst, 19, 12) << 12, 20);
#define IMM_U uint32_t imm = sub_bits(this->inst, 31, 12);
...
void RV32Core::do_sra() {
    RD; RS1; RS2;
    this->set_gpr(rd, (int32_t)this->gpr[rs1] >> this->gpr[rs2]);
    this->npc = this->pc + 4;
}

void RV32Core::do_slt() {
    RD; RS1; RS2;
    this->set_gpr(rd, (int32_t)this->gpr[rs1] < (int32_t)this->gpr[rs2] ? 1 : 0);
    this->npc = this->pc + 4;
}
...
void RV32Core::do_srai() {
    RD; RS1; IMM_I;
    this->set_gpr(rd, (sword_t)this->gpr[rs1] >> imm);
    this->npc = this->pc + 4;
}

void RV32Core::do_slti() {
    RD; RS1; IMM_I;
    this->set_gpr(rd, (sword_t)this->gpr[rs1] < imm ? 1 : 0);
    this->npc = this->pc + 4;
}

void RV32Core::do_sltiu() {
    RD; RS1; IMM_IU;
    this->set_gpr(rd, this->gpr[rs1] < imm ? 1 : 0);
    this->npc = this->pc + 4;
}
...
void RV32Core::do_lbu() {
    RD; RS1; IMM_I;
    this->set_gpr(rd, this->memory->read(this->gpr[rs1] + imm, 1));
    this->npc = this->pc + 4;
}
...
void RV32Core::do_lhu() {
    RD; RS1; IMM_I;
    this->set_gpr(rd, this->memory->read(this->gpr[rs1] + imm, 2));
    this->npc = this->pc + 4;
}
...
void RV32Core::do_bge() {
    RS1; RS2; IMM_B;
    if ((int32_t)this->gpr[rs1] >= (int32_t)this->gpr[rs2]) {
        this->npc = this->pc + imm;
    } else {
        this->npc = this->pc + 4;
    }
